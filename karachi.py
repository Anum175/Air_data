...
def fetch_historical_aqi_hourly(lat, lon):
    ...
    # Save data to a file
    with open("karachi_aqi_hourly.txt", "w") as f:
        for entry in data:
...
"""
            f.write(entry_text)
    print(f"Saved {len(data)} hourly data points to karachi_aqi_hourly.txt")


# Fetch AQI data for Karachi (coordinates: 24.8607, 67.0011)
fetch_historical_aqi_hourly(24.8607, 67.0011)
