...
def fetch_historical_aqi_hourly(lat, lon):
    ...
    # Save data to a file
    with open("islamabad_aqi_hourly.txt", "w") as f:
        for entry in data:
...
"""
            f.write(entry_text)
    print(f"Saved {len(data)} hourly data points to islamabad_aqi_hourly.txt")


# Fetch AQI data for Islamabad (coordinates: 33.6844, 73.0479)
fetch_historical_aqi_hourly(33.6844, 73.0479)
